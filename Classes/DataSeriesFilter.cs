...
namespace SimpleJsonDataSource
{
	...
	{
		public static IEnumerable<DataPointByMeasure<double>> FilterDataPoints(this IEnumerable<DataPointByMeasure<double>> dataPoints, DateTime startDateTime, DateTime endDateTime, TimeSpan samplingInterval, TimeSpan smoothingWindow)
		{
		...
		private static IEnumerable<DataPointByMeasure<double>> GetDataPointEnumerable((double Value, double Count)[] points, DateTime startDateTime, TimeSpan samplingInterval)
		{
			int pointIndex;
			DateTime pointDateTime;
		...
		private static DataPointByMeasure<double> CreateDataPoint(ref (double Value, double Count) point, DateTime dateTime)
			=> new DataPointByMeasure<double>(dateTime, point.Count > 0 ? point.Value / point.Count : 0);
